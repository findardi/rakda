...
export const load: PageServerLoad = async ({ locals, params, parent, url, depends }) => {
	...
	const closed = { forbidden: false, notViewable: false, pending: false, failed: false };
	...
	if (!res.ok) {
		...
		if (res.status === 403) return { meta: null, versions: [], ...closed, forbidden: true };
		// 422 is structural: an unsupported file type, or a document above the
		// page cap. A rendition that failed or is still being prepared arrives
		// as 200 with its status — there is no sentinel message to match on.
		if (res.status === 422) return { meta: null, versions, ...closed, notViewable: true };
		if (res.status === 404) error(404, t('doc.view.err.notFound'));
		error(res.status || 500, t('doc.view.err.load'));
	}

	const status = res.data?.rendition_status ?? 'ready';
