...
- **Link preload policy**: body stays `data-sveltekit-preload-data="hover"`. Every link into the document viewer carries `off` (its load hits `GET /view`, which writes `document_viewed` — a view must be a completed click, not even `tap`); room rows (`/workspace` list, `RoomSidebar` switcher) carry `tap` (one row = a 6–7-call room subtree load). Folder-rail and sidebar links stay `hover`; `/api/...` download links are never preloaded by SvelteKit. Any new viewer link must carry `off` — including the engagement page's open-document button and per-page table links; the activity timeline's read links go to the engagement page (not the viewer) and keep `hover`.
...
- Audit (`activity` domain): actions → `activity_logs` (same-tx `RecordTx` inside `ExecTxTx`, else best-effort `Record`; consumers declare an `ActivityRecorder` port); page views/read durations → `content_events` (append-only, no FK to documents, snapshotted names/actors). Two tables, never merged, never UPDATEd. Owner/admin produce **no** `content_events` — filtered on the write side (`content_view_service.go` skips `RecordPageEvent`, `RecordPageDurations` no-ops, client builds no dwell tracker) so a promoted guest keeps their history; `activity_logs` still covers every role. Timeline/engagement endpoints are owner/admin only — guests are recorded, never readers. Engagement is per-reader, two levels (L1 readers → L2 that reader's pages); no cross-reader page heatmap, not even per group. It lives on its own route `/workspace/{slug}/engagement/{folderId}/{documentId}` (mirrors `/view`, loader redirects guests), reached from the document row's ⋮ menu and the timeline's read links — never inside the viewer (side panel removed 2026-09-05). Engagement responses carry `page_count` of the served version (so the page never calls `GET /view`) and each reader's **current** group (`group_id`/`group_name`, empty once they left — events snapshot the actor, not the group); the web groups the reader list by it, the CSV carries `group_name`. Selected reader rides `?reader=` (layout load = list, page load = pages).
- **Guest invitations may carry a group** (`workspace_user_invitations.group_id`, nullable, `on delete set null`, migration 00039): guest-only (non-guest role + group → 400), must belong to the room (else 404), and the actor's authority over the role (`guardRoleAssignment`) is checked before the group. Both accept paths (`access.ConsumeInvitation`, `invitation.AcceptInvitation`) run `AssignToGroup` before `AssignDefaultGroupIfGuest`; the PK on `workspace_group_members(member_id)` + `on conflict do nothing` makes the chosen group win and the default fill in only when none was chosen or the group was deleted meanwhile.
- **Guest access expiry** (`workspace_members.expires_at`, nullable, migration 00040): optional, guest-only (owner/admin + date → 400), NULL = never, **no default duration**. Set at invite (`workspace_user_invitations.access_expires_at`, copied on both accept paths; a closed window refuses acceptance like a lapsed link) or via `PATCH .../members/{memberID}/expiry` (`member:edit`, `{expires_at: RFC3339 | null}`). Enforced by one clause in `GetMembershipWithPermissions` (`expires_at is null or > now()`) — the query all four resolvers call, so fail-closed without touching `Membership` — plus the workspace-list queries. Lapse is derived (`memberStatus` → `expired`), never stored, no sweeper; row/group/history stay and a new date or a clear revives access. Audit `member_expiry_changed` `{from,to}`; the lapse itself is not an action. `GET .../me` returns the caller's `expires_at` for the sidebar.
- **Room branding** (migration 00041: `workspaces.logo_key`, `workspaces.hero_preset`, both nullable = today's generative default): owner-only mutations, member-only visibility, nothing pre-login. Logo goes **through the API** (`PUT /workspaces/{id}/branding/logo`, multipart), never presigned: `platform/brandimage.NormalizeLogo` sniffs bytes (PNG/JPEG/WebP; SVG refused by content — inline SVG = stored XSS), caps dimensions before decoding (16 MP), scales to 512 px, re-encodes PNG (drops EXIF). 2 MB → 413, format → 415, unreadable → 400, archived → 423. Stored at `asset/logo/{ws}/{uuid}.png` (top-level family, never age-swept); object → row → delete-old ordering, failed row deletes the object, `DeleteWorkspace` drops the prefix. `GET .../branding/logo` is member-gated via `GetWorkspaceForMember` with `ETag`/304 + `private, max-age=86400`, proxied by `routes/api/workspaces/[id]/branding/logo`; `logo` in the response is the uuid cache token. Hero background = curated preset only (`GET /workspaces/hero-presets`, `PUT .../branding/hero`, empty = automatic; hue/phase served resolved as `hero_hue`/`hero_phase`, `autoHero` mirrors the old web hash). Overview hero is the only tenant-imagery surface; tiles elsewhere show the logo only. Audit `workspace_branding_changed` `{kind, action|preset}`. Deal tier, not gated yet.
...
- **Gate = two `r.Use` middlewares on HTTP method, never a permission classification.** `RequireRoomOpenForGuests` (guest+`prepare` → 403) and `RequireRoomWritable` (`archive`+non-GET → **423**) in all **five** `{workspaceID}` modules (workspace included since 2026-09-03), all sharing one resolver, `access/repository.(*Repository).ResolveMembership` — never copy it into a module (U-11 closed); status rides `GetMembershipWithPermissions` via one PK join (zero extra queries, zero `granted` CTE edits). Method, not permission: `GET /folder-templates/` sits behind `folder:create` and `GET .../multipart/parts` behind `document:upload`, so a permission gate would 423 pure reads. Never push the guard below the handler — `GET /view` writes (`renditionState` → `RequestRendition` + worker wake, `promoteStaged`, page cache, `RecordPageEvent`), so a lower guard makes archived rooms unreadable. Four router-sibling exceptions: `POST /search/log`, `POST .../duration`, `POST .../retry-rendition` (else a once-failed rendition is permanently unreadable), `POST .../archives`, plus `PATCH /workspaces/{id}/status` (leaving `archive` is the one mutation an archived room must accept). `PUT /workspaces/{id}` audits `workspace_updated` `{from,to,description_changed}` same-tx and writes nothing when nothing changed. Transitions guarded (→`prepare` and same-to-same → 409) and audited once as `workspace_status_changed` `{from,to}`; rejections never audited.
- **Archive export = stored artifact, not a stream.** Manager-only, every room status, async into the top-level `archives/` prefix (separate from `renditions/` so its 30-day TTL can never reach `rendition.pdf`). `workspace_archives` (`pending`/`ready`/`failed`) is not a job queue — same shape as `staged_version_id`+`rendition_failed_at`: one status row, one goroutine, manual retry, sweeper fails rows a deploy left hanging. Download sends `Content-Length` + `Accept-Ranges` so interrupted transfers **resume** — the reason it is stored at all, since the 300 s proxy cap is a bandwidth cliff. Browser must use a plain `<a href>`, never `fetch`+blob. Contents: clean renditions, folder tree with cumulative dotted number prefixes baked into names, clickable `_indeks.html` + machine-readable `_indeks.csv`, `_audit/` CSVs. Failed renditions are reported in the index, never silently dropped. **Scope (2026-09-04):** `scope_folder_ids` + `scope_folder_names` snapshot (migration 00042; NULL = whole room; check constraint refuses `'{}'`). `POST /archives {folder_ids?}` — overview modal sends checked root ids (all checked → none), rail ⋮ sends one id at any depth; each id = its subtree; validated against `ListArchiveFolders` before insert (unknown → 404, `[]` → 400). Walk covers the whole tree, numbers + dedup suffixes over all siblings, only in-scope nodes emitted → scoped ZIP is a strict subset of the whole-room ZIP (gaps mark exclusions). `_audit/` only in the whole-room package; README states scope + omission. Names `{slug}-{folder}-arsip-{date}` / `{slug}-arsip-sebagian-{date}`; `Content-Disposition` via `mime.FormatMediaType`. One `pending` per room still (409). `archive_exported` metadata: `scope`, `folder_count`, `folder_names`. Web: `common/ActionMenu.svelte` (native Popover API, top layer) holds every row action but `+`; zip item gated on role only, never `writable`; posts cross-route to the overview `?/createArchive` after a short confirm.
